...
export default class WxUtils {
  ...
  static backOrAction(url, actionType) {
    url = this.mapUrl(url);
    if (this.isTab(url)) {
      wx.switchTab({
        url: url
      ...
      if (pages.length < 2 || index < 0) {
        if (actionType == ActionTypes.redirect) {
          wx.redirectTo({
            url: url
          });
  ...
  static backOrRedirect(url) {
    this.backOrAction(url, actionTypes.redirect)
  }
  /**
   * 如果能够后退（多层），则navigaetBack，否则调用navigateTo
   */
  static backOrNavigate(url) {
    this.backOrAction(url, actionTypes.navigate)
  }
